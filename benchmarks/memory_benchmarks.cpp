...
struct CacheLineBenchmark
{
  ...
  static void
  cache_friendly_task(std::vector<int>& data, size_t start_idx, size_t count)
  {
...
static void
BM_CacheFriendly_HighPerformancePool(benchmark::State& state)
{
...
static void
BM_CacheUnfriendly_HighPerformancePool(benchmark::State& state)
{
  size_t const num_threads = state.range(0);
...
static void
BM_MemoryAllocation_TaskCreation(benchmark::State& state)
{
  size_t const num_threads = state.range(0);
...
static void
BM_NUMA_LocalMemory(benchmark::State& state)
{
  size_t const num_threads = state.range(0);
...
static void
BM_FalseSharing_Avoided(benchmark::State& state)
{
  size_t const num_threads = state.range(0);
