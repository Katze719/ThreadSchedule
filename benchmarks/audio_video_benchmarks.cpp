...
private:
  ...
  static void
  apply_lowpass_filter(std::vector<float>& left, std::vector<float>& right)
  {
  ...
  static void
  apply_highpass_filter(std::vector<float>& left, std::vector<float>& right)
  {
  ...
  static void
  apply_noise_reduction(std::vector<float>& left, std::vector<float>& right)
  {
          ...
          [&]()
            {
              for (size_t frame_num = 0;
                   frame_num < fps * stream_duration_seconds; ++frame_num)
                {
                  ...
                  // Check if queue is getting full (simulate real-time
                  // constraints)
