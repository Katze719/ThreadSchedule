 ...
 * submit/post/try_submit/try_post API surface as thread_pool_backend so it can
 * be used as a drop-in template argument in generic code that is parameterized
 * on pool type.
 */
 ...
 * inline_pool_backend provides the same submit/try_submit/post/try_post/
 * submit_batch/parallel_for_each surface as @ref thread_pool_backend. The
...
{
public:
  explicit inline_pool_backend(size_t /*num_threads*/ = 0) {}
      ...
      -> std::future<std::invoke_result_t<F, Args...>>
  {
    ...
    if (!result.has_value())
      throw std::runtime_error("inline_pool_backend is shut down");
  ...
  [[nodiscard]] auto
  pending_tasks() const noexcept -> size_t
  {
    return 0;
  }
