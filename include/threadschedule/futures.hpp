...
/**
 * @brief Block until all futures complete, returning results in submission
 * order.
 *
...
template <typename T>
auto
when_all(std::vector<std::future<T>>& futures) -> std::vector<T>
{
 ...
 */
inline void
when_all(std::vector<std::future<void>>& futures)
{
  std::exception_ptr first_error;
...
/**
 * @brief Block until all void futures complete, returning an @c expected per
 * slot.
 */
inline auto
...
template <typename T>
auto
when_any(std::vector<std::future<T>>& futures) -> std::pair<size_t, T>
{
  if (futures.empty())
 ...
 */
inline auto
when_any(std::vector<std::future<void>>& futures) -> size_t
{
  if (futures.empty())
    throw std::invalid_argument("when_any: empty futures vector");
