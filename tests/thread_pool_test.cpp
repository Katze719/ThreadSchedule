...
// ==================== thread_pool_backend Basic Tests ====================
...
{
  auto pool = std::make_unique<thread_pool_backend>(2);
  std::atomic<int> counter{ 0 };
...
// ==================== work_stealing_pool_backend Tests ====================
...
TEST_F(ThreadPoolTest, HighPerformancePoolConfigureThreads)
{
  ...
  [[maybe_unused]] auto config_result = pool.configure_threads("worker");
...
TEST_F(ThreadPoolTest, HighPerformancePoolDistributeAcrossCPUs)
{
  ...
  [[maybe_unused]] auto distribution_result = pool.distribute_across_cpus();
...
{
  work_stealing_pool_backend pool(1); // Single thread to create queue
...
// ==================== polling_pool_backend Tests ====================

TEST_F(ThreadPoolTest, FastThreadPoolCreation)
{
  polling_pool_backend pool(4);
  // Pool created successfully
  EXPECT_TRUE(true);
}
...
TEST_F(ThreadPoolTest, PerformanceComparisonSimpleTasks)
{
  ...
  std::cout << "thread_pool_backend: " << time1 << " ms" << std::endl;

  work_stealing_pool_backend pool2(num_threads);
  auto time2 = test_pool(pool2);
  std::cout << "work_stealing_pool_backend: " << time2 << " ms" << std::endl;

  polling_pool_backend pool3(num_threads);
  auto time3 = test_pool(pool3);
  std::cout << "polling_pool_backend: " << time3 << " ms" << std::endl;
