...
main()
{
  ...
  size_t a
      = registry()
            .filter([](auto const& e) { return e.component == "RuntimeLibA"; })
            .count();
  size_t b
      = registry()
            .filter([](auto const& e) { return e.component == "RuntimeLibB"; })
            .count();
  ...
  bool has_libA = registry().any([](auto const& e)
                                   { return e.component == "RuntimeLibA"; });
  bool has_libB = registry().any([](auto const& e)
                                   { return e.component == "RuntimeLibB"; });
  ...
  auto found_a
      = registry().find_if([](auto const& e) { return e.name == "ra-1"; });
  auto found_b
      = registry().find_if([](auto const& e) { return e.name == "rb-1"; });
  ...
  if (!found_a)
    ...
    {
      std::cout << "  Found LibA thread: " << found_a->name
                << " (TID: " << found_a->tid << ")\n";
    }
  ...
  if (!found_b)
    ...
    {
      std::cout << "  Found LibB thread: " << found_b->name
                << " (TID: " << found_b->tid << ")\n";
    }
