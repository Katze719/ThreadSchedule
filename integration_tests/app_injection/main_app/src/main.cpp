...
main()
{
  ...
  thread_registry_backend app_registry;
  bool success = true;
  ...
  size_t a
      = registry()
            .filter([](auto const& e) { return e.component == "AppInjLibA"; })
            .count();
  size_t b
      = registry()
            .filter([](auto const& e) { return e.component == "AppInjLibB"; })
            .count();
  ...
  auto found
      = registry().find_if([](auto const& e) { return e.name == "inj-a1"; });
  if (!found)
    {
