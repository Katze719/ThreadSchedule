...
#include <iostream>
#include <thread>
#include <threadschedule/advanced/composite_thread_registry.hpp>
...
main()
{
  ...
  threadschedule::advanced::composite_thread_registry registries;
  registries.attach(composite_libA::get_registry());
  registries.attach(composite_libB::get_registry());
  auto merged = registries.snapshot();
  if (!merged)
    return 1;
  ...
  auto const found
      = std::find_if(merged->begin(), merged->end(), [](auto const& entry) { return entry.name == "ca-1"; });
