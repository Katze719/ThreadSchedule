...
main()
{
  ...
  if (a_count != 2)
    {
      std::cerr << "  ERROR: Expected 2 threads in LibA registry, got "
                << a_count << "\n";
      success = false;
    }
  if (b_count != 2)
    {
      std::cerr << "  ERROR: Expected 2 threads in LibB registry, got "
                << b_count << "\n";
      success = false;
    }
  ...
  bool libA_tags_ok = composite_libA::get_registry().all(
      [](auto const& e) { return e.component == "CompositeLibA"; });
  bool libB_tags_ok = composite_libB::get_registry().all(
      [](auto const& e) { return e.component == "CompositeLibB"; });
  ...
  if (total != 4)
    {
      std::cerr << "  ERROR: Expected 4 threads in composite registry, got "
                << total << "\n";
      success = false;
    }
  ...
  size_t comp_a = comp.filter([](auto const& e)
                                { return e.component == "CompositeLibA"; })
                      .count();
  size_t comp_b = comp.filter([](auto const& e)
                                { return e.component == "CompositeLibB"; })
                      .count();
  ...
  if (!found)
    ...
    {
      std::cout << "  Found thread: " << found->name << " from "
                << found->component << "\n";
    }
  ...
  if (first_two.size() != 2)
    {
      std::cerr << "  ERROR: Expected 2 entries from take(2), got "
                << first_two.size() << "\n";
      success = false;
    }
  ...
  std::cout << "  Final counts: composite=" << final_total
            << ", A=" << (libA_empty ? "empty" : "not empty")
